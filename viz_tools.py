...
def fourier_transform(signal, N, T):
    ...
    consisting of <N> data points and an upper sample time <T>"""
...
def eta_animation(X, Y, eta_list, frame_interval, filename):
    ...
    pmesh = plt.pcolormesh(X, Y, eta_list[0], cmap=plt.cm.RdBu_r)
    plt.colorbar(pmesh, orientation="vertical")

    def update_eta(num):
        # We directly work with the Z array suitable for the expected dimensions.
        Z = eta_list[num][:-1, :-1]  # Adjust Z to match X, Y dimensions for pcolormesh
        # Flatten Z and remove the last column of the last row to fit the expected array size
        pmesh.set_array(Z[:-1, :-1].ravel())
        ax.set_title(f"Surface elevation $\\eta$ after t = {num*frame_interval/3600:.2f} hours", fontname="serif", fontsize=16)
        return pmesh,
